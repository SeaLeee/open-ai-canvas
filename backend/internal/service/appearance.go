...
const (
	defaultAppearanceBrandName = "ShotFlow"
	defaultAppearanceBrandSlug = "shotflow"
	...
	defaultAppearanceHeroTitle = "从分镜到成片，\n一步到位。"
)
...
func (s *Service) ResetAppearance(actor *model.User) (*AdminAppearanceSetting, error) {
	...
	if err := s.appendAdminAudit(actor, "appearance.reset", "system_setting", appearanceSettingKey, "恢复 ShotFlow 默认品牌标识", map[string]any{"before": before, "after": after}); err != nil {
		return nil, err
	}
	return s.AdminAppearance(actor)
}
